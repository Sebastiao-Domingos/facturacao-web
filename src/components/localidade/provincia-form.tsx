// src/components/localidade/provincia-form.tsx
"use client";
  ...
  async function onSubmit(data: Provincia) {
    try {
      ...
      if (
        ...
        form.setError("nome", {
          type: "manual",
          message: "Já existe uma província com este nome",
        });
      } else {
        form.setError("root", {
          type: "manual",
                  ...
                  <div className="flex justify-between items-center min-h-[20px]">
                    {fieldState.error ? (
                      ...
                      <span className="text-xs text-muted-foreground/50">
                        Nome oficial da província conforme divisão
                        administrativa
                      </span>
                    )}
        ...
        <DialogFooter className="border-t border-border bg-muted/20 px-6 py-4 gap-3">
          ...
          >
            {isLoading ? (
              ...
              <>
                <Save size={16} />
                {isEditing ? "Atualizar Província" : "Criar Província"}
              </>
            )}
          </Button>
        </DialogFooter>
